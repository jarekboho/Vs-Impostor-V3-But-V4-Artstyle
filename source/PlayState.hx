...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (!PlayStateChangeables.Optimize)
		{
			...
			//	add(stageCurtains);
				...
				speaker.antialiasing = true;
						...
						deadBF = new FlxSprite(600, 525).loadGraphic(Paths.image('polus/bfdead', 'impostor'));
		                                deadBF.setGraphicSize(Std.int(deadBF.width * 0.8));
		...
		gf = new Character(1200, 175, curGf);
		if(curSong == 'Ejected') {
		...
		dad = new Character(265, 178, SONG.player2);
		...
		boyfriend = new Boyfriend(770, 490, SONG.player1);
		...
		switch (curStage)
		{
			...
			case 'school':
				...
				dad.setPosition(350, 100);
				gf.setPosition(650, 310);
				dad.y += 496;
				dad.x += 113;
				boyfriend.x += -5;
				boyfriend.y += 375;
				if(SONG.song.toLowerCase() == 'chewmate') {
				dad.y += 14;
				dad.x += 20;
				boyfriend.x += -12;
				boyfriend.y += 8;
		...
		healthBar.scrollFactor.set();
                healthBar.createFilledBar(FlxColor.fromString('#' + dad.playerColor), FlxColor.fromString('#' + boyfriend.playerColor));
		// healthBar
		add(healthBar);
